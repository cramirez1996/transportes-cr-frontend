...
export class TransactionExplorerComponent implements OnInit, OnDestroy {
  ...
  groupedTimeSeriesData: GroupedTimeSeriesResult | null = null;
  ...
  // Granularity (synced with form)
  selectedGranularity: TimeGranularity = TimeGranularity.MONTH;
  ...
  constructor(
    private transactionExplorerService: TransactionExplorerService,
  ...
  syncFormToFilters(formValue: any): void {
    ...
    // Update granularity
    this.selectedGranularity = formValue.granularity;
   ...
   * Uses grouped time series (one line per entity based on groupBy)
   */
  loadTimeSeries(): void {
    ...
    const params = {
      ...
      vehicleIds: this.filters.vehicleIds,
      driverIds: this.filters.driverIds,
      customerIds: this.filters.customerIds,
      supplierIds: this.filters.supplierIds,
      paymentMethod: this.filters.paymentMethod,
      limit: 10, // Top 10 entities by total amount
    };

    console.log('loadTimeSeries - Filters:', this.filters);
    console.log('loadTimeSeries - Params:', params);

    this.transactionExplorerService.getGroupedTimeSeries(params).subscribe({
      next: (data) => {
        console.log('loadTimeSeries - Response:', data);
        this.groupedTimeSeriesData = data;
        this.timeSeriesData = null; // Clear traditional data
        this.isLoadingChart = false;
      },
      error: (error) => {
        console.error('Error loading grouped time series:', error);
        this.isLoadingChart = false;
      },
    });
  }
  ...
  loadAggregate(): void {
    ...
    console.log('loadAggregate - Filters:', this.filters);
    console.log('loadAggregate - Params:', params);

    this.transactionExplorerService.getAggregate(params).subscribe({
      next: (data) => {
        console.log('loadAggregate - Response:', data);
  ...
  loadComparison(): void {
    ...
    this.transactionExplorerService.comparePeriods(params).subscribe({
      next: (data) => {
